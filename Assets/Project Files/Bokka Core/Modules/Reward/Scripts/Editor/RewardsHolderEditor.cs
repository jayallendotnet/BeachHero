    ...
    public class RewardsHolderEditor : Editor
    {
        ...
        private static void InitializeTypes()
        {
            ...
            foreach (Assembly assembly in assemblies)
            {
                if (assembly != null)
                {
                    IEnumerable<Type> types = assembly.GetTypes().Where(x => x.IsClass && !x.IsAbstract && x.IsSubclassOf(typeof(Reward)));
        ...
        private void OnDisable()
        {
            ...
            if(target == null)
            {
                ...
                if(!selectedObjects.IsNullOrEmpty())
                {
                    ...
                    {
                        RewardsHolder holder = selectedObject.GetComponent<RewardsHolder>();
                        if(holder == null)
                        {
                            Undo.RecordObject(selectedObject, "Clear rewards list");

                            Reward[] rewardComponents = selectedObject.GetComponents<Reward>();
                            foreach(Reward rewardComponent in rewardComponents)
                            {
        ...
        private void UpdateOffers()
        {
            ...
            rewards = rewardsHolder.GetComponents<Reward>();
